...
def train_td(gamma=1, alpha=0.1, episodes=200000):
	...
	for episode in range(episodes):
		...
		for i in range(1,len(states)):
			...
			## This is saying the next q_value is equal to the next state, and previous action
			next_q = q_table[states[i][0]][states[i][1]][states[i][2]][int(actions[i])]

			## Temporal difference update
			q_table[state[0]][state[1]][state[2]][int(actions[i])] = current_q + alpha*(rewards[i-1] + gamma*next_q - current_q)
